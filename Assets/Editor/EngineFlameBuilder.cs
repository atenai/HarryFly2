...
public static class EngineFlameBuilder
{
	...
	/// <summary>
	/// 噴射口の位置。機体モデルの頂点を測って求めた値。
	///
	/// 後端（z=-0.743）付近は左右の張り出しが 0.033 しかない細い筒で、これが噴射口にあたる。
	/// その高さの範囲は y 0.272〜0.337、中心が 0.305。
	///
	/// 以前は z=-1.28 と、後端から 0.54 も後ろに置いていた。
	/// カメラは機体の2.5後方・高さ0.5から見ているので、そこまで下げると
	/// 炎のほうがカメラに近くなり、同じ高さでも遠近で画面上は下にずれて見える
	/// （視線に対し後端が -6.3 度なのに対して、炎は -9.3 度）。
	/// 高さを水増しして辻褄を合わせると、今度は噴射口より上から出ているように見える。
	/// 炎そのものを噴射口へ移すのが正しい直し方だった。
	///
	/// 後ろへ出しすぎるとカメラと機体の間に入って画面を覆う
	/// </summary>
	static readonly Vector3 NozzlePosition = new Vector3(0f, 0.305f, -0.78f);
